import { Avatar, Box, Divider, IconButton, ListItemIcon, Menu, MenuItem, Tab, Tabs, Tooltip, useTheme } from "@mui/material";
...
export default function MainMenu(){
    ...
    let [hoverIndicator, setHoverIndicator] = useState({left: 0, width: 0, hovered: false, initial: true});
    let theme = useTheme();
    let onTabMouseIn = (e) => {
        let menu = e.target.closest(".main-menu");
        let tabBox = e.target.getBoundingClientRect();
        let menuBox = menu.getBoundingClientRect();
        setHoverIndicator({left: tabBox.left - menuBox.left, width:tabBox.width, hovered: true, initial: hoverIndicator.initial ? (hoverIndicator.hovered ? false : true) : false });
    };
    let onTabMouseOut = () => {
    ...
    return <div className="w-full flex gap-4 items-end">
            <Box className="main-menu flex-grow" sx={{ borderBottom: 1, borderColor: 'divider', position: "relative" }}>
                <Tabs className="mb-[-1px]" value={tab} onChange={(e, v) => setTab(v)}>
                    ...
                    <motion.div className="absolute bottom-0 h-[2px]" style={{width: hoverIndicator.width + "px"}}
                        animate={{left: hoverIndicator.left}}
                        transition={{duration: hoverIndicator.initial ? 0 : 0.2}}
                    >
                        <motion.div className="h-full m-auto" style={{background: theme.palette.secondary.main}}
                            animate={{width: hoverIndicator.hovered ? "100%" : "0%" }}
